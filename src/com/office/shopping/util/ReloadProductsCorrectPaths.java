 ...
 * @version 1.0
 */
public class ReloadProductsCorrectPaths {
    ...
    public static void main(String[] args) {
        try {
            ...
            for (File file : pngFiles) {
                ...
                product.setDescription(description);  // 使用完整文件名作为描述，用于匹配图片
                ...
                count++;
                System.out.println(String.format("%2d. %-20s -> %s", count, name, category));
            }
    ...
    private static List<File> getPNGFiles(String dirPath) {
        ...
        if (files != null) {
            for (File file : files) {
                if (file.isFile() && file.getName().toLowerCase().endsWith(".png") 
                    && !file.getName().contains("haven't_photo")) {
                    pngFiles.add(file);
                }
            }
        }
    ...
    private static String simplifyName(String name) {
        ...
        simplified = simplified.replace("Apple苹果", "苹果");
        ...
        simplified = simplified.replace("软面抄", "");
        ...
        simplified = simplified.replace("Zebra斑马", "斑马");
        ...
        simplified = simplified.replace("便签纸", "便签");
